// ===================== [FTIM-1] components/FakeTradeImageManager.tsx =====================
...
type Props = {
  fakeTradeId: number | null;        // null = NEW (cola local)
  userId: string;
  readOnly?: boolean;
...
export default function FakeTradeImageManager({
  fakeTradeId,
  userId,
  readOnly = false,
  onQueueChange,
  ...
  useEffect(() => {
    if (readOnly) {
      if (fakeTradeId) loadFromDb();
      else setRows([]);
      return;
    }
    if (!fakeTradeId) rebuildLocalRows();
    else loadFromDb();
  }, [readOnly, fakeTradeId, rebuildLocalRows, loadFromDb]);
  ...
  async function handleBlobs(blobs: Blob[]) {
    ...
    try {
      for (const blob of blobs) {
        ...
        const ins = await supabase.from("fake_trade_images").insert({
          user_id: userId,
  ...
  async function addByUrl() {
    ...
    try {
      const { error } = await supabase.from("fake_trade_images").insert({
        user_id: userId,
        fake_trade_id: fakeTradeId,
        title: "url",
        external_url: src,
...
async function remove(id?: number, src?: string) {
  ...
  try {
    ...
    if (img?.storage_path) {
      const rm = await supabase.storage.from("journal").remove([img.storage_path]);
      if (rm.error) throw rm.error;
    }

    const { error } = await supabase.from("fake_trade_images").delete().eq("id", id);
    if (error) throw error;

    await loadFromDb();
  } catch (e: any) {
    console.error("fake_trade_images delete error", e);
    alert("Error borrando imagen: " + (e?.message ?? e));
  }
}
                ...
                {!readOnly && (
                  <div className="img-meta" style={{ display: "grid", gap: 6 }}>
                    ...
                    <button
                      ...
                      onClick={() => (fakeTradeId ? remove(r.id) : remove(undefined, r.external_url ?? r.blobUrl))}
                    >
                      Borrar
                    </button>
                  </div>
